...
standard_fields = ['Reference','Identifier','Footprint','Value','Tolerance','Voltage','Current','Power','PN']
extra_fields = []

i=1
# Gather the components' information
for c in soup.components.find_all("comp"):
  #c is an XML tag representing each component. Pull known fields out.
  ...
  new_comp['Power'] = ''
  new_comp['PN'] = ''
  #For each key in the component, if it's not in the standard or extra field
  #list, add it to the extra field list
  ...
  found = None
  #Build up a dictionary which matches new_comp but doesn't 
  #include the Reference key
  new_comp_no_ref = {}
...
with codecs.open(sys.argv[2],"w", encoding='utf-8') as f:
  f.write("Reference,Quantity,Identifier,Footprint,Value,Tolerance,Voltage,Current,Power,PN")
  ...
  for k in comp.keys():
    ...
    l = [ref, str(len(c['Reference'])), c['Identifier'], c['Footprint'], c['Value'], c['Tolerance'], c['Voltage'], c['Current'], c['Power'], c['PN']]
